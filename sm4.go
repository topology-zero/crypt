...
func (c *Sm4Cipher) CBCEncrypt(dst, src []byte) {
	iv := c.iv
	for i := 0; i < len(src)/16; i++ {
		inTmp := xor(src[i*16:i*16+16], iv)
		outTmp := make([]byte, 16)
		cryptBlock(c.subKeys, c.block1, c.block2, outTmp, inTmp, false)
		copy(dst[i*16:i*16+16], outTmp)
		iv = outTmp
	}
}

func (c *Sm4Cipher) CBCDecrypt(dst, src []byte) {
	iv := c.iv
	for i := 0; i < len(src)/16; i++ {
		inTmp := src[i*16 : i*16+16]
		outTmp := make([]byte, 16)
		cryptBlock(c.subKeys, c.block1, c.block2, outTmp, inTmp, true)
...
func (c *Sm4Cipher) ECBEncrypt(dst, src []byte) {
	for i := 0; i < len(src)/16; i++ {
		inTmp := src[i*16 : i*16+16]
		outTmp := make([]byte, 16)
		cryptBlock(c.subKeys, c.block1, c.block2, outTmp, inTmp, false)
...
func (c *Sm4Cipher) ECBDecrypt(dst, src []byte) {
	for i := 0; i < len(src)/16; i++ {
		inTmp := src[i*16 : i*16+16]
		outTmp := make([]byte, 16)
		cryptBlock(c.subKeys, c.block1, c.block2, outTmp, inTmp, true)
...
func (c *Sm4Cipher) CFBEncrypt(dst, src []byte) {
	K := make([]byte, c.BlockSize())
	cipherBlock := make([]byte, c.BlockSize())
	for i := 0; i < len(src)/16; i++ {
		if i == 0 {
			cryptBlock(c.subKeys, c.block1, c.block2, K, c.iv, false)
			cipherBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
			copy(dst[i*16:i*16+16], cipherBlock)
			continue
		}
		cryptBlock(c.subKeys, c.block1, c.block2, K, cipherBlock, false)
		cipherBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
		copy(dst[i*16:i*16+16], cipherBlock)
	}
}

func (c *Sm4Cipher) CFBDecrypt(dst, src []byte) {
	K := make([]byte, c.BlockSize())
	plainBlock := make([]byte, c.BlockSize())
	for i := 0; i < len(src)/16; i++ {
		if i == 0 {
			cryptBlock(c.subKeys, c.block1, c.block2, K, c.iv, false)
			plainBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
			copy(dst[i*16:i*16+16], plainBlock)
			continue
		}
		cryptBlock(c.subKeys, c.block1, c.block2, K, src[(i-1)*16:(i-1)*16+16], false)
		plainBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
		copy(dst[i*16:i*16+16], plainBlock)
	}
}

func (c *Sm4Cipher) OFBEncrypt(dst, src []byte) {
	K := make([]byte, c.BlockSize())
	IV := make([]byte, c.BlockSize())
	cipherBlock := make([]byte, c.BlockSize())
	for i := 0; i < len(src)/16; i++ {
		if i == 0 {
			cryptBlock(c.subKeys, c.block1, c.block2, K, IV, false)
			cipherBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
			copy(dst[i*16:i*16+16], cipherBlock)
			copy(IV, K[:c.BlockSize()])
			continue
		}
		cryptBlock(c.subKeys, c.block1, c.block2, K, IV, false)
		cipherBlock = xor(K[:c.BlockSize()], src[i*16:i*16+16])
...
func (c *Sm4Cipher) OFBDecrypt(dst, src []byte) {
	K := make([]byte, c.BlockSize())
	IV := make([]byte, c.BlockSize())
	plainBlock := make([]byte, c.BlockSize())
	for i := 0; i < len(src)/16; i++ {
